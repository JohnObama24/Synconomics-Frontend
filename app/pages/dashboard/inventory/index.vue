<template>
  <div class="space-y-8">
    ...
    <div
      class="bg-syn-dark border border-syn-border rounded-2xl overflow-hidden flex flex-col glow-border relative min-h-[400px]">

      <div v-if="isLoading"
        class="absolute inset-0 bg-syn-darker/50 backdrop-blur-sm z-10 flex items-center justify-center">
      ...
      <div v-else class="divide-y divide-syn-border bg-syn-dark">
        <div v-for="item in filteredProducts" :key="item.id"
          class="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-syn-surface transition-colors group">

          <div class="col-span-1">
            <div
              class="w-10 h-10 rounded-xl bg-syn-surface flex items-center justify-center border border-syn-border group-hover:border-syn-accent/30 transition-colors overflow-hidden">
          ...
          <div class="col-span-4">
            <h4 class="text-sm font-semibold text-syn-cream group-hover:text-syn-accent transition-colors">{{ item.name
              }}</h4>
          ...
          <div class="col-span-2">
            <span
              class="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-syn-surface text-syn-muted border border-syn-border">
          ...
          <div class="col-span-2 text-right flex flex-col items-end justify-center">
            <span :class="['text-sm font-bold mb-1.5', item.stock > 10 ? 'text-syn-cream' : 'text-syn-warning']">{{
              item.stock.toLocaleString() }}</span>
            <div class="h-1.5 w-16 bg-syn-border rounded-full overflow-hidden">
              <div :class="['h-full rounded-full', item.stock > 10 ? 'bg-syn-success' : 'bg-syn-warning']"
                :style="{ width: Math.min(100, (item.stock / 100) * 100) + '%' }"></div>
            </div>
          </div>

          <div
            class="col-span-1 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <button @click="handleEdit(item)"
              class="p-1.5 flex text-syn-muted hover:text-syn-cream bg-syn-surface rounded-md border border-syn-border hover:border-syn-accent/30 transition-colors">
              <Icon name="heroicons:pencil-square" class="w-4 h-4" />
            </button>
            <button @click="handleDelete(item.id)"
              class="p-1.5 flex text-syn-muted hover:text-red-400 bg-syn-surface rounded-md border border-syn-border hover:border-red-500/30 transition-colors">
    ...
    <!-- Modal -->
    <DashboardProductFormModal v-if="showModal" :businessId="activeBusinessId" :initialData="selectedProduct"
      @close="closeModal" @saved="onProductSaved" />
  </div>
</template>
...
const filteredProducts = computed(() => {
  ...
  return products.value.filter(product =>
    ...
    `PROD-${product.id}`.toLowerCase().includes(query)
  );
});
